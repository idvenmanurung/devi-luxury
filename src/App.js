 ...
 * VERSION: 39.5.0 (ULTRA-INSTANT LOAD OPTIMIZATION)
 * ==========================================================================================
 */
        ...
        <main>
          {view === 'shop' && (
            <div className="animate-in fade-in duration-200">
              ...
              <ProductGrid products={filteredProducts} isLoading={isLoadingProducts} onView={(p) => { setSelectedProduct(p); setView('detail'); window.scrollTo(0,0); }} />
              <MembershipBanner />
            </div>
          )}
...
function HeroSection({ onExplore }) {
  return (
    <section className="relative h-[45vh] md:h-[70vh] flex items-center justify-center overflow-hidden bg-zinc-950 font-bold uppercase">
      <img src="https://images.unsplash.com/photo-1549439602-43ebcb232811?q=80&w=2070&auto=format&fit=crop" className="absolute inset-0 w-full h-full object-cover opacity-60 font-bold" alt="Hero" />
      <div className="relative z-10 text-center text-white px-6 font-bold uppercase animate-in slide-in-from-bottom duration-500">
...
function ProductGrid({ products, isLoading, onView }) {
  ...
  return (
    <section id="catalog" className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-2 lg:grid-cols-4 gap-6 font-bold uppercase min-h-[400px]">
      {products.map(p => (
        <div key={p.id} className="cursor-pointer group flex flex-col items-center font-bold animate-in fade-in duration-300" onClick={() => onView(p)}>
          <div className="aspect-[3/4] w-full rounded-2xl overflow-hidden bg-zinc-50 mb-3 shadow-sm group-hover:shadow-lg transition-all font-bold relative">
            <img src={p.imageURLs?.[0] || p.imageURL} className="w-full h-full object-cover group-hover:scale-105 transition-all duration-700" alt={p.name} loading="lazy" />
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 font-bold uppercase">
        <div className="space-y-4 font-bold uppercase">
          ...
          <div className="flex gap-3 overflow-x-auto no-scrollbar font-bold uppercase">
            {images.map((img, i) => (
              <div key={i} onClick={() => setActiveImg(i)} className={`w-16 h-16 rounded-xl overflow-hidden flex-shrink-0 cursor-pointer border-2 transition-all ${activeImg === i ? 'border-[#D4AF37]' : 'border-transparent'}`}><img src={img} className="w-full h-full object-cover font-bold uppercase" /></div>
            ))}
          </div>
        </div>
