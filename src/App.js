 ...
 * VERSION: 36.0.0 (FIXED BRI LOGO TO 12.PNG & 5 GALLERY SLOTS)
 * ==========================================================================================
 */
...
const BANK_LOGOS = {
  ...
  // UPDATE: Menggunakan 12.png sesuai instruksi terbaru
...
// Fungsi Kompresi untuk mempercepat upload dari HP
  ...
  return new Promise((resolve) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        resolve(canvas.toDataURL('image/jpeg', 0.6)); // 60% quality agar loading sangat cepat
      };
    };
  });
};
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-[100] h-14 md:h-16 bg-white border-b border-zinc-100 flex items-center px-4 font-bold uppercase text-black">
      <div className="max-w-7xl mx-auto w-full flex items-center justify-between font-bold">
        ...
        <div className="flex-1 flex justify-end gap-4 items-center font-bold">
        ...
        <div className="space-y-8 font-bold uppercase">
          ...
          {product.showAgeSelection !== false && (
            <div className="space-y-4 font-bold uppercase">
              <label className="text-[10px] text-zinc-400 font-bold tracking-widest uppercase">PILIH UMUR</label>
          ...
          <div className="space-y-4 font-bold uppercase">
            <label className="text-[10px] text-zinc-400 font-bold tracking-widest uppercase">PILIH UKURAN</label>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 md:py-16 font-bold uppercase text-black">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 items-start font-bold uppercase">
        <div className="lg:col-span-2 space-y-8 font-bold uppercase">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3 font-bold uppercase">
                {rekening.map(rek => (
                  <div key={rek.id} onClick={()=>setPayment({...payment, transferTo: `${rek.bankName} - ${rek.accountNumber} - ${rek.accountHolder}`})} className={`p-4 rounded-xl border-2 cursor-pointer flex flex-col items-center gap-2 transition-all font-bold uppercase ${payment.transferTo.includes(rek.accountNumber) ? 'border-black bg-zinc-100' : 'border-zinc-100'}`}>
                    <img src={BANK_LOGOS[rek.bankName]} className="h-6 object-contain font-bold uppercase" />
          ...
          {step === 5 && (
            <div className="text-center py-20 animate-in zoom-in font-bold uppercase font-bold">
              <div className="w-20 h-20 bg-green-500 text-white rounded-full flex items-center justify-center mx-auto mb-6 shadow-xl shadow-green-100 font-bold uppercase"><SuccessIcon size={40}/></div>
...
function AdminDashboard({ products, orders, rekening, shippingMethods, appId, onLogout, notify, creds }) {
  ...
  const publishProduct = async () => {
    ...
    try {
      ...
      if (editingId) {
        ...
        notify("Katalog berhasil dipublish.", "success");
      }
      resetForm();
    } catch(e) { 
      notify("Terjadi kesalahan: " + e.message, "error"); 
    } finally { 
      setSaving(false); 
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 flex flex-col md:flex-row gap-8 font-bold uppercase text-black">
          ...
          <div className="bg-white w-full max-w-xl rounded-3xl overflow-hidden shadow-2xl animate-in zoom-in font-bold uppercase">
            ...
            <div className="p-8 space-y-6 max-h-[85vh] overflow-y-auto no-scrollbar text-black uppercase font-bold">
              ...
              <div className="text-[10px] bg-zinc-50 p-4 rounded-xl font-bold uppercase">
                ...
                <p className="font-bold uppercase leading-relaxed font-bold">{selectedOrder.address}, {selectedOrder.city}</p>
              </div>
              <div>
      ...
      <div className="flex-1 bg-white p-6 rounded-3xl border min-h-[60vh] shadow-sm font-bold uppercase text-black font-bold uppercase">
        {tab === 'inventory' && (
          <div className="space-y-10 font-bold uppercase font-bold uppercase">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 border-b pb-10 font-bold uppercase">
              <div className="space-y-6 font-bold uppercase font-bold uppercase">
                <div className="space-y-3 font-bold uppercase font-bold uppercase">
                  ...
                  <div className="grid grid-cols-3 gap-3 font-bold uppercase font-bold">
              ...
              <div className="space-y-4 font-bold uppercase">
                ...
                <button onClick={publishProduct} disabled={saving} className="w-full bg-black text-[#D4AF37] py-5 rounded-2xl font-bold text-[10px] shadow-lg tracking-widest uppercase font-bold uppercase">{saving ? 'PUBLISHING...' : 'SIMPAN SEKARANG'}</button>
              </div>
            </div>
                ...
                <div key={p.id} className="border rounded-xl overflow-hidden relative group font-bold uppercase">
                  ...
                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center gap-4 font-bold">
                    <button onClick={()=>{
                      ...
                      setTab('inventory'); 
                      window.scrollTo(0,0);
                    }} className="p-2 bg-white rounded-full text-black font-bold font-bold"><Edit size={16}/></button>
                    <button onClick={async()=>{if(confirm('Hapus produk?')) await deleteDoc(doc(db,'artifacts',appId,'public', 'data', 'products',p.id));}} className="p-2 bg-red-500 text-white rounded-full font-bold font-bold"><Trash2 size={16}/></button>
                  </div>
                  <div className="p-2 bg-white font-bold uppercase"><p className="text-[8px] font-bold truncate uppercase font-bold">{p.name}</p></div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-4 font-bold uppercase">
            ...
            {orders.map(o => (
              <div key={o.id} onClick={()=>setSelectedOrder(o)} className="p-4 bg-zinc-50 rounded-2xl flex justify-between items-center cursor-pointer hover:bg-zinc-100 transition-all font-bold">
                <div className="flex items-center gap-4 font-bold uppercase">
                   <img src={o.proofImage} className="w-10 h-14 rounded-lg object-cover font-bold uppercase" />
                ...
                <button onClick={async(e)=>{e.stopPropagation(); if(confirm('Hapus order?')) await deleteDoc(doc(db,'artifacts',appId,'public','data','orders',o.id));}} className="text-zinc-300 hover:text-red-500 bg-transparent border-none cursor-pointer font-bold font-bold"><Trash2 size={16}/></button>
              </div>
            ))}
          </div>
        )}
          ...
          <div className="space-y-8 font-bold uppercase">
            <div className="bg-zinc-50 p-6 rounded-3xl space-y-4 font-bold uppercase shadow-inner">
              ...
              <form onSubmit={addBank} className="space-y-4 font-bold uppercase">
                ...
                <button type="submit" className="w-full bg-black text-[#D4AF37] py-4 rounded-xl font-bold text-[9px] uppercase font-bold font-bold font-bold uppercase">TAMBAH METODE</button>
              </form>
            </div>
              ...
              {rekening.map(rek => (
                <div key={rek.id} className="p-4 border rounded-2xl flex justify-between items-center shadow-sm font-bold uppercase">
                  <div className="flex items-center gap-3 font-bold uppercase">
                    <img src={BANK_LOGOS[rek.bankName]} className="h-4 w-10 object-contain font-bold uppercase"/>
                  ...
                  <button onClick={async()=>await deleteDoc(doc(db,'artifacts',appId,'public','data','rekening',rek.id))} className="text-red-500 border-none bg-transparent cursor-pointer font-bold uppercase font-bold"><Trash2 size={14}/></button>
                </div>
              ))}
            </div>
          </div>
        ...
        {tab === 'shipping' && (
          <div className="space-y-6 uppercase font-bold uppercase font-bold">
             ...
             <div className="bg-zinc-50 p-8 rounded-3xl space-y-4 font-bold uppercase">
                {localShipping.map((m, idx) => (
                  <div key={m.id} className="flex items-center gap-4 bg-white p-4 rounded-xl shadow-sm border font-bold uppercase">
                     <div className="w-16 h-10 bg-white p-1 rounded-lg border border-zinc-100 flex items-center justify-center overflow-hidden font-bold uppercase">
                       <img src={m.logo} className="w-full h-full object-contain" alt={m.name} />
                     </div>
                     <span className="flex-1 text-[10px] font-bold uppercase font-bold uppercase">{m.name}</span>
                     <div className="flex items-center gap-2 font-bold uppercase font-bold uppercase">
                        <span className="text-[9px] text-zinc-400 uppercase font-bold font-bold uppercase font-bold uppercase">Rp</span>
                        <input 
                          type="number" 
                          className="w-24 p-2 bg-zinc-50 rounded-lg text-[10px] font-bold font-bold uppercase font-bold" 
                          value={m.price} 
                          onChange={(e) => {
                ...
                }} className="w-full bg-black text-[#D4AF37] py-4 rounded-xl font-bold text-[9px] mt-4 shadow-xl uppercase font-bold uppercase font-bold uppercase">UPDATE TARIF</button>
             </div>
          </div>
        )}
        ...
        {tab === 'settings' && (
          <div className="max-w-md space-y-6 font-bold uppercase font-bold uppercase">
            <h3 className="text-xs font-serif italic border-b pb-2 uppercase font-bold font-bold uppercase">Pengaturan Akses</h3>
            <div className="bg-zinc-50 p-8 rounded-3xl space-y-6 shadow-inner font-bold uppercase font-bold uppercase">
              <div className="space-y-1 font-bold uppercase font-bold uppercase font-bold">
                <label className="text-[9px] text-zinc-400 uppercase font-bold">Username Login Baru</label>
                <input className="w-full p-4 rounded-xl border-none text-[10px] font-bold uppercase font-bold uppercase font-bold" value={newCreds.username} onChange={e=>setNewCreds({...newCreds, username:e.target.value})}/>
              </div>
              <div className="space-y-1 font-bold uppercase font-bold uppercase font-bold">
                <label className="text-[9px] text-zinc-400 uppercase font-bold">Password Login Baru</label>
                <input className="w-full p-4 rounded-xl border-none text-[10px] font-bold uppercase font-bold uppercase font-bold" type="password" value={newCreds.password} onChange={e=>setNewCreds({...newCreds, password:e.target.value})}/>
              </div>
              <button onClick={updateAdminAuth} className="w-full bg-black text-[#D4AF37] py-4 rounded-xl font-bold text-[9px] shadow-lg uppercase font-bold font-bold uppercase">UPDATE PROFIL ADMIN</button>
            </div>
          </div>
        )}
      </div>
...
function CartView({ items, onRemove, onCheckout }) {
  ...
  return (
    <div className="max-w-3xl mx-auto py-16 px-4 font-bold uppercase text-black font-bold uppercase font-bold uppercase">
       <div className="text-center mb-12 space-y-2 animate-in slide-in-from-bottom font-bold uppercase font-bold">
          <h2 className="text-4xl font-serif italic uppercase font-bold uppercase font-bold font-bold uppercase">Maison <span className="text-[#D4AF37]">Bag</span></h2>
       ...
       {items.length === 0 ? (
         <div className="text-center py-32 border-2 border-dashed border-zinc-100 rounded-[2.5rem] bg-zinc-50/30 flex flex-col items-center font-bold uppercase font-bold">
            <ShoppingBag size={64} className="text-zinc-100 mb-6 font-bold uppercase font-bold uppercase" />
            <p className="text-zinc-300 font-bold text-[10px] tracking-widest uppercase font-bold uppercase font-bold font-bold">Bag Kosong</p>
         </div>
       ) : (
         <div className="space-y-6 font-bold uppercase font-bold uppercase font-bold">
            {items.map((item, idx) => (
              <div key={idx} className="p-4 bg-white border border-zinc-100 rounded-3xl flex items-center justify-between gap-6 shadow-sm font-bold uppercase font-bold font-bold">
                 <div className="flex items-center gap-5 flex-1 font-bold uppercase font-bold font-bold uppercase">
                    <img src={item.imageURLs?.[0] || item.imageURL} className="w-20 h-24 rounded-2xl object-cover shadow-md border border-zinc-50 font-bold uppercase font-bold uppercase"/>
                    <div className="space-y-1.5 flex-1 font-bold uppercase font-bold font-bold uppercase">
                       <h4 className="text-[11px] font-serif font-bold uppercase tracking-tight text-zinc-800 font-bold uppercase font-bold font-bold uppercase">{String(item.name).toUpperCase()}</h4>
                       <div className="flex gap-2 font-bold uppercase font-bold font-bold uppercase"><span className="text-[7px] font-black px-2 py-0.5 bg-zinc-100 rounded-full border border-zinc-200 uppercase font-bold uppercase font-bold uppercase">Umur {String(item.chosenAge)}</span><span className="text-[7px] font-black px-2 py-0.5 bg-zinc-100 rounded-full border border-zinc-200 uppercase font-bold uppercase font-bold uppercase">Size {String(item.chosenSize)}</span></div>
                       <p className="text-xs font-bold text-black italic font-bold uppercase font-bold uppercase font-bold uppercase font-bold">{formatIDR(item.chosenPrice || item.price)}</p>
                    </div>
                 </div>
                 <button onClick={()=>onRemove(idx)} className="p-3 text-zinc-300 hover:text-red-500 bg-zinc-50 rounded-2xl border-none cursor-pointer font-bold uppercase font-bold font-bold font-bold uppercase"><Trash2 size={18}/></button>
              </div>
            ))}
            <div className="pt-10 border-t border-zinc-100 flex flex-col md:flex-row justify-between items-center gap-10 font-bold uppercase font-bold font-bold font-bold uppercase">
               <div className="text-center md:text-left space-y-0.5 font-bold uppercase font-bold uppercase font-bold">
                  <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest uppercase font-bold font-bold font-bold uppercase">Grand Total</p>
                  <p className="text-4xl md:text-6xl font-serif font-black italic tracking-tighter text-zinc-950 uppercase font-bold font-bold uppercase font-bold">{formatIDR(total)}</p>
               </div>
               <button onClick={onCheckout} className="w-full md:w-auto bg-black text-[#D4AF37] px-16 py-6 rounded-full font-bold uppercase text-[11px] tracking-widest shadow-2xl border-none cursor-pointer flex items-center justify-center gap-4 uppercase font-bold font-bold font-bold uppercase font-bold">Checkout Sekarang <ArrowRight size={20} /></button>
            </div>
         </div>
       )}
    </div>
...
function Footer({ setView }) {
  return (
    <footer className="bg-[#030303] text-white pt-20 pb-10 px-6 border-t-[4px] border-[#D4AF37] font-bold uppercase font-bold font-bold">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-12 font-bold uppercase font-bold uppercase">
        <div className="space-y-6 text-center md:text-left font-bold uppercase font-bold font-bold uppercase">
           <h2 className="text-2xl md:text-4xl font-serif font-bold italic tracking-widest text-[#D4AF37] leading-none uppercase font-bold uppercase font-bold uppercase font-bold">DEVI OFFICIAL</h2>
           <p className="text-zinc-500 text-[10px] leading-relaxed italic opacity-70 font-bold uppercase font-bold font-bold uppercase">Elevating modest fashion to a global standard of absolute luxury. Kemewahan abadi berawal dari tanggung jawab sosial dalam setiap produksi.</p>
        </div>
        <div className="space-y-6 hidden md:block font-bold uppercase font-bold font-bold uppercase">
           <h4 className="text-[10px] font-bold uppercase tracking-[0.4em] text-zinc-100 border-b border-white/5 pb-2 uppercase font-bold uppercase font-bold uppercase font-bold">Concierge</h4>
           <div className="text-[9px] text-zinc-500 tracking-widest space-y-2 italic leading-none font-bold uppercase font-bold font-bold">
              <p className="flex items-center gap-2 uppercase font-bold uppercase font-bold font-bold"><MapPin size={14} className="text-[#D4AF37]"/> Jakarta, Indonesia</p>
              <p className="flex items-center gap-2 uppercase font-bold uppercase font-bold font-bold"><Phone size={14} className="text-[#D4AF37]"/> +62 852-2444-4164</p>
              <p className="flex items-center gap-2 uppercase font-bold uppercase font-bold font-bold"><Mail size={14} className="text-[#D4AF37]"/> boutique@devi-official.com</p>
           </div>
        </div>
        <div className="space-y-6 text-center md:text-left font-bold uppercase font-bold font-bold uppercase">
           <h4 className="text-[10px] font-bold uppercase tracking-[0.4em] text-zinc-100 border-b border-white/5 pb-2 uppercase font-bold uppercase font-bold uppercase font-bold">Maison Bank</h4>
           <div className="flex flex-wrap gap-4 opacity-30 grayscale hover:opacity-100 transition-all cursor-pointer font-bold uppercase font-bold font-bold uppercase">
              {Object.values(BANK_LOGOS).slice(0, 4).map((l, i) => <img key={i} src={l} className="h-4 object-contain font-bold uppercase font-bold font-bold uppercase" alt="" />)}
           </div>
        </div>
      </div>
      <div className="max-w-7xl mx-auto mt-16 pt-8 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-6 font-bold uppercase font-bold font-bold uppercase">
         <div className="text-[8px] uppercase tracking-[0.4em] italic opacity-30 font-bold font-bold uppercase font-bold uppercase font-bold uppercase">© 2024 DEVI_OFFICIAL LUXURY GROUP INC. ALL RIGHTS RESERVED.</div>
         <button onClick={() => setView('login')} className="flex items-center gap-2 text-zinc-700 text-[9px] tracking-widest hover:text-[#D4AF37] transition-all border border-white/5 px-6 py-2 rounded-full bg-zinc-950 cursor-pointer shadow-inner uppercase font-bold font-bold uppercase font-bold uppercase font-bold">
